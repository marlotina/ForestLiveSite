...
export class UserPostService {
  ...
  getPostsByLabel(userId: string, label: string){
    return this.httpClient.get<PostListResponse[]>(`${environment.userPostApiUrl}api/v1/BirdUser/GetPostsByLabel/?label=${label}&userId=${userId}`)
      .pipe(map(data => {
        return data;
      }));
  }
  ...
  getPostsByUser(userId: string){
    return this.httpClient.get<PostListResponse[]>(`${environment.userPostApiUrl}api/v1/BirdUser/GetPosts/?userId=${userId}`)
      .pipe(map(data => {
        return data;
      }));
  }
